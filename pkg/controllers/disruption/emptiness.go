...
//
//nolint:gocyclo
func (e *Emptiness) ComputeCommand(ctx context.Context, budget *Budget, candidates ...*Candidate) (Command, scheduling.Results, error) {
	if e.IsConsolidated() {
	...
	for _, candidate := range candidates {
		...
		if budget.Capacity(candidate.nodePool.Name, e.Reason()) == 0 {
		...
		budget.Allocate(candidate.nodePool.Name, e.Reason(), candidate.Node.Name)
	}
